...
d, n = 2, 9
data = Point{d,Float64}[2*rand(Point{d,Float64}) for i in 1:n]  # create the list of points for the tree test
push!(data,Point(2.0,2.0))        # The is the the top right corner
push!(data,Point(0.0,0.0))        # This is the lower left corner
push!(data, Point(0.90,0.5))      # This point is at the edgre of sector(0) (-,-)
...
push!(data, Point(0.90,0.5))      # now we add another point but with slightly fatter raduis than the ratio 1.2
push!(radii,0.26)
